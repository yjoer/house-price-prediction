...
# Validation predictions and metrics
score_val <- mean(dt$results$RMSE)
...
### Stacking
```{r}
stacked_data <- data.frame(y = y_test, prediction_rf = y_pred_test_rf, prediction_xgb = y_pred_test_xgb, prediction_lgb = y_pred_test_lgb, prediction_catboost = y_pred_test_catboost)
model_meta <- caret::train(y ~ ., data = stacked_data, method = "lm")
predictions_meta <- predict(model_meta, newdata = stacked_data)

ensemble_rmse_test$stacking_score <- rmse(y_pred_test, y_test)
ensemble_actual_metrics_test$stacking <- metrics_fusion(y_pred_test, y_test)
